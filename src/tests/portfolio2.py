...
class BenchmarkNeutralOptimizer:
    ...
    def factor_tilt_penalty(self, w, tilt_target, lambda_tilt, sector):
        """Calculates the factor tilt penalty given weights, tilt target, and regularization strength."""
        beta = self.combine_betas(sector)

        # Ensure alignment between beta and weights
        common_tickers = beta.index.intersection(w.index)
        if common_tickers.empty:
            raise ValueError("No common tickers between portfolio weights and factor betas.")

        beta_aligned = beta.loc[common_tickers]
        w_aligned = w.loc[common_tickers]

        # Compute factor exposures for the portfolio
        result = beta_aligned.T @ w_aligned  # This is now a vector

        # Ensure tilt_target is properly formatted as a vector
        if tilt_target is None:
            tilt_target = np.zeros_like(result)  # If None, set to a zero vector
        else:
            tilt_target = np.asarray(tilt_target)  # Convert to NumPy array for vectorized operations

        # Compute penalty term as a scalar
        penalty = lambda_tilt * np.sum((result - tilt_target) ** 2)
    ...
    def calculate_tilt(self, sector=None):
        ...
        w_b = self.benchmark_w(sector)
        betas = self.combine_betas(sector)

        # Ensure alignment between benchmark weights and betas
        common_tickers = w_b.index.intersection(betas.index)
        if common_tickers.empty:
            raise ValueError("No common tickers between benchmark weights and factor betas.")

        # Align indices explicitly to avoid unintended broadcasting
        w_b_aligned = w_b.loc[common_tickers]
        betas_aligned = betas.loc[common_tickers]

        # Compute tilt target
        tilt = betas_aligned.T @ w_b_aligned
